...
import { getTopServices, getDateRange } from "@/lib/analytics/calculators";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const metrics = await getTopServices(
      supabase,
      businessId,
      range.start,
      range.end
    ...
    console.error("[analytics/services]", err);
    return jsonResponse({ error: "Failed to compute service analytics" }, { status: 500 });
  }
}
